...
def extract_sections(text):
    ...
    for section in splited[1:]:
        apartado = re.match(patternApartado, section).group(0).strip()
        contenido = re.sub(patternApartado, "", section)
        sections[apartado] = contenido.strip()
    return sections
...
def info_sections(data):
    ...
    promptCriterios = prompt.format(
        ...
        query="Imprime una lista Python en formato [{'Nombre': None, 'Siglas': None, 'Puntuación máxima': None, 'Puntuación mínima': None}] con los criterios de adjudicación de precio, juicio de valor, fórmulas, entre otros, separados por comas.",
    )
    promptSubcriterios = prompt.format(
        section=sections["APARTADO LL"],
        query="Imprime una lista Python en formato [{'Nombre': None, 'Puntuación máxima': None}] con todos los subcriterios dentro de 'Criterios relacionados con la calidad cuya cuantificación depende de un juicio de valor' o los apartados de la memoria.",
    )
    ...
    promptDetPrecio = prompt.format(
        ...
        query="Indica únicamente el tipo de determinación del precio. Si hay varios, incluirlos. La X significa que esa es la opción seleccionada",
    )
    promptAbonoCuentas = prompt.format(
        section=sections["APARTADO W"],
        query="Indica si se exigen abonos a cuenta (Sí o No). Si no aparece, devolver None",
    )
    ...
    promptInfraccionGrave=prompt.format(
        section=sections["APARTADO T"],
        query="Indica la consideración como infracción grave del inclumplimiento de las condiciones especiales de ejecución (Sí o No). Si no aparece, devolver None",
    )
    ...
    promptContratacionControl=prompt.format(
        ...
        query="Indica si hay CONTRATACIÓN DEL control de calidad de la obra mediante un contrato independiente (Sí o No). Si no aparece, devolver None",
    )
    dictInfo["CONTRATACION DEL CONTROL"]=llm.invoke(promptContratacionControl).content
    promptGastos=prompt.format(
        section=sections["APARTADO Y"],
        query="Indica el texto del apartado de IMPORTE DE GASTOS POR DESISTIMIENTO O RENUNCIA. Si no aparece, devolver None",
    )
    dictInfo["GASTOS POR DESISTIMIENTO O RENUNCIA"]=llm.invoke(promptGastos).content
    promptInclusionControl=prompt.format(
        section=sections["APARTADO Y"],
        query="Indica si hay INCLUSIÓN DEL CONTROL DE CALIDAD EN LA PROPIA OBRA (Sí o No). Si no aparece, devolver None",
    )
    ...
    promptMejora=prompt.format(
        section=sections["APARTADO LL"],
        query="Indica si hay MEJORAS COMO CRITERIO DE ADJUDICACIÓN (Sí o No). Si no aparece, devolver None",
    )
    ...
    promptObligacionSubcontrata=prompt.format(
        ...
        query="Indica si hay OBLIGACIÓN DE INDICAR EN LA OFERTA SI VA A HABER SUBCONTRATACIÓN (Sí o No). Si no aparece, devolver None",
    )
    ...
    promptPenalidades=prompt.format(
        section=sections["APARTADO T"],
        query="Indica las Penalidades por incumplimiento de las condiciones especiales de ejecución. En caso de que no aparezcan devolver None",
    )
    ...
    promptPlazoRecepcion=prompt.format(
        ...
        query="Indica el plazo de recepción. Si no aparece, devolver None",
    )
    ...
    promptRevision=prompt.format(
        ...
        query="Indica si hay revisión de precios y el motivo. Si no aparece, devolver None",
    )
    dictInfo["REVISION DE PRECIOS"]=llm.invoke(promptRevision).content
    promptSubasta=prompt.format(
        section=sections["APARTADO H"],
        query="Indica si hay subasta electrónica (Sí o No). Si no aparece, devolver None",
    )
    dictInfo["SUBASTA ELECTRONICA"]=llm.invoke(promptSubasta).content
    promptTareasCriticas=prompt.format(
        section=sections["APARTADO Q"],
        query="Indica si hay TAREAS CRITICAS QUE NO PODRÁN SER OBJETO DE SUBCONTRATACIÓN (Sí o No) y si las hay, cuáles. Si no aparece, devolver None",
    )
